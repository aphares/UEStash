...
void AWorldBuilder::OverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult) {
	...
	if (mapRef && collider) {
		...
		if (collLoc.X > GetActorLocation().X && collLoc.X < GetActorLocation().X + chunkSize && collLoc.Y > GetActorLocation().Y && collLoc.Y < GetActorLocation().Y + chunkSize && collider->GetLocalRole() >= ROLE_AutonomousProxy) mapRef->spawnGrid(collLoc, collider);
	}
}
...
void AWorldBuilder::OnGenerateMesh_Implementation() {
	...
	Chunks = GetRealtimeMeshComponent()->InitializeRealtimeMesh<UChunker>();
	//mapRef->setMaterial(Chunks); // ubiquitous across all grids
	Chunks->AddLOD(FRealtimeMeshLODConfig());
	Chunks->AddLOD(FRealtimeMeshLODConfig());
	Chunks->SetBounds(mapRef, GetActorLocation() / mapRef->mapResolution);
